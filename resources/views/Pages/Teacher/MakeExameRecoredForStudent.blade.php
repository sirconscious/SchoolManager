    ...
    <ul class="flex flex-wrap text-sm font-medium text-center text-gray-500 border-b border-gray-200 dark:border-gray-700 dark:text-gray-400 mb-1 absolute top-20 left-64">
        ...
        <li class="me-2">
            <a href="{{ route('record.create') }}" 
               class="inline-block p-4 rounded-t-lg 
               {{ request()->routeIs('record.create') || request()->routeIs('record.createS') ? 'text-blue-600 bg-gray-100 dark:bg-gray-800 dark:text-blue-500' : 'hover:text-gray-600 hover:bg-gray-50 dark:hover:bg-gray-800 dark:hover:text-gray-300' }}">
                Add Record
            </a>
        </li>
...
<form class="max-w-sm mx-auto w-full" action="{{route('record.store')}}" method="POST">
    ...
    <div class="mb-5">
        ...
        <div>
            <label for="" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Student name</label>
            <input type="text" name="users_id" id="users_id" value="{{$student->id}}" readonly hidden class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"  placeholder="{{$student->name}}" required />
            <input type="text" id="" value="{{$student->name}}" readonly  class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"  placeholder="{{$student->name}}" required />
        </div>
            <div class="mt-2">
